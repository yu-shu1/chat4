...
window.addEventListener('load', function() {
    setTimeout(function() {
        try {
            ...
            var storageKey = 'dailyOpenCount';
            var lastDateKey = 'dailyOpenLastDate';
            
            // 获取当前存储的日期和次数
            var lastDate = localStorage.getItem(lastDateKey);
            var openCount = parseInt(localStorage.getItem(storageKey) || '0', 10);
            ...
            // 增加本次打开次数
            openCount++;
            localStorage.setItem(storageKey, openCount);
            ...
            if (openCount <= 7) {
                ...
                // 可选：记录已弹出（原代码有 dailyGreetingShown，这里不再需要，因为用计数器控制）
                // localStorage.setItem('dailyGreetingShown', today);
            }
