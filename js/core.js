    ...
    setTimeout(() => {
        ...
        function getDefaultSettings() {
            return {
                ...
                partnerTime: Date.now(),
                partnerNextInterval: 6 * 3600000 + Math.random() * 24 * 3600000,
...
const createMessage = (imgSrc = null) => {
    ...
    if (!isBatchMode && type === 'normal') {
        ...
        if (!shouldIgnore) {
            showTypingIndicator();
            // 移除之前的 scrollTop 操作，因为 showTypingIndicator 内部已处理定位
            // 如果你需要滚动到底部，可以在 showTypingIndicator 中做，或保留：
...
function showTypingIndicator() {
    ...
    // 清除之前的隐藏定时器
    if (typingIndicatorState.hideTimer) {
        clearTimeout(typingIndicatorState.hideTimer);
        typingIndicatorState.hideTimer = null;
    }

    // 如果已经显示，不再重复操作
    if (typingIndicatorState.visible) return;

    // 设置文案（支持自定义）
    let customText = localStorage.getItem('tiSettings_customText') || '';
    if (customText) {
        label.textContent = customText;
    } else {
        const name = settings.partnerName || '对方';
        label.textContent = name + ' 正在输入';
    }

    // 设置头像（根据设置显示或隐藏）
    const showAvatar = localStorage.getItem('tiSettings_showAvatar') !== 'false';
    if (showAvatar) {
    ...
    if (indicator) indicator.classList.remove('hiding');

    typingIndicatorState.visible = true;
}
...
function hideTypingIndicator(immediate = false) {
    ...
    if (immediate) {
        ...
        if (indicator) indicator.classList.remove('hiding');
        typingIndicatorState.visible = false;
        return;
    }

    if (!typingIndicatorState.visible) return;

    if (indicator) indicator.classList.add('hiding');
    typingIndicatorState.hideTimer = setTimeout(() => {
        wrapper.style.display = 'none';
        if (indicator) indicator.classList.remove('hiding');
        typingIndicatorState.visible = false;
        typingIndicatorState.hideTimer = null;
