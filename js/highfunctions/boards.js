...
if (isMultiSelectMode) {
    ...
    // 确保删除按钮存在并绑定事件
    let deleteBtn = document.getElementById('board-delete-selected-btn');
    if (!deleteBtn) {
        deleteBtn = document.createElement('button');
        deleteBtn.id = 'board-delete-selected-btn';
        deleteBtn.className = 'board-multi-action-btn';
        deleteBtn.innerHTML = '<i class="fas fa-trash"></i> 删除';
        multiBar.appendChild(deleteBtn);
    }
    // 绑定点击事件（每次刷新都重新绑定，防止重复）
    const delBtn = document.getElementById('board-delete-selected-btn');
    if (delBtn) {
        delBtn.onclick = () => {
            if (selectedThreadIds.size === 0) {
                if(typeof showNotification === 'function') showNotification('请至少选择一条留言', 'warning');
            ...
            if(confirm(`确定要删除选中的 ${selectedThreadIds.size} 条留言吗？此操作不可恢复。`)) {
...
async function deleteSelectedThreads() {
    ...
    const threads = isMe ? boardData.myThreads : boardData.partnerThreads;
    const newThreads = threads.filter(t => !selectedIds.includes(t.id));
    if (isMe) {
        boardData.myThreads = newThreads;
    } else {
        boardData.partnerThreads = newThreads;
    }
    await saveData();
    if (typeof showNotification === 'function') {
        showNotification(`已删除 ${selectedIds.length} 条留言`, 'success');
    }
    exitMultiSelectMode(); // 退出多选并刷新列表
}
