...
function _renderModernToolbar() {
    ...
    if (_searchVisible) {
        ...
        toolbar.querySelector('#tb-search-clear').onclick = () => { _searchVisible = false; _searchQuery = ''; renderReplyLibraryRaf(); };
    }
        ...
        panel.querySelectorAll('[data-action]').forEach(btn => {
            btn.onclick = () => {
                ...
                if (action === 'toggle') {
                    ...
                    throttledSaveData(); render(); renderReplyLibraryRaf();
                } else if (action === 'edit') {
                    overlay.remove();
...
function _showImportUI(data) {
    ...
    _showIOSheet(`导入字卡`, `文件中包含 ${modules.length} 个模块`, modules, ICONS.import, (selected, mode) => {
        ...
        try {
            ...
            throttledSaveData();
            if (typeof renderReplyLibrary === 'function') renderReplyLibraryRaf();
