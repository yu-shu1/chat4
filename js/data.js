(function () {
    ...
    function applyStats(total, msgs, cfg, media) {
        var pct = Math.min(100, total / (100 * 1024 * 1024) * 100);
        ...
        if (g('dm-storage-total')) g('dm-storage-total').textContent = fmt(total) + ' / ~100 MB';
        ...
        if (window.localforage && window.APP_PREFIX) {
            localforage.keys().then(function(keys) {
                ...
                Promise.all(promises).then(function(sizes) {
                    ...
                    if (text) text.textContent = fmt(total) + ' / ~100 MB (' + pct + '%)';
                });
            }).catch(function() {
                var ls = 0;
                for (var i = 0; i < localStorage.length; i++) {
                ...
                var pct = Math.min(ls / (100*1024*1024) * 100, 100).toFixed(1);
